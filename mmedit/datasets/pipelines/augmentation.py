    ...
    def __call__(self, results):
        ...
        if flip:
            for key in self.keys:
                ...
                #             for direct_key in v.keys():
                #                 mmcv.imflip_(v[direct_key], self.direction)
                #     else:
                #         for direct_key in results[key].keys():
                #             mmcv.imflip_(results[key][direct_key], self.direction)
                # else:
                if isinstance(results[key], list):
                    for v in results[key]:
    ...
    def __call__(self, results):
        ...
        if transpose:
            for key in self.keys:
                # if key=='mvs':
                #     if isinstance(results[key], list):
                #         for v in range(len(results[key])):
                #             for direct_key in results[key][v].keys():
                #                 results[key][v][direct_key] = results[key][v][direct_key] .transpose(1, 0, 2)
                #     else:
                #         for direct_key in results[key].keys():
                #                 results[key][direct_key] = results[key][direct_key] .transpose(1, 0, 2)
...
@PIPELINES.register_module()
class GenerateFrameIndiceswithPaddingEDVR:
    def __init__(self,
                 padding,
                 filename_tmpl='{:08d}.png',
                 filename_tmpl_gt=None,
                 idx_start_from=1):
    ...
    def __call__(self, results):
        ...
        frame_name=int(frame_name.split('.png')[0])
        

        current_idx = frame_name - self.idx_start_from
        last_idx = results['sequence_length'] - 1  # start from 0
        ...
        for i in range(current_idx - num_pad, current_idx + num_pad + 1):
            ...
            if i < 0:
                ...
                if self.padding == 'replicate':
                    ...
                    pad_idx = last_idx * 2 - i
                elif self.padding == 'reflection_circle':
                    pad_idx = (current_idx - num_pad) - (i - last_idx)
                else:
        ...
        lq_paths = [
            osp.join(
                ...
                f'{self.filename_tmpl.format(idx + self.idx_start_from)}')
            for idx in frame_list
        ]
        if self.filename_tmpl_gt:
            gt_paths = [osp.join(gt_path_root, clip_name, f'{self.filename_tmpl_gt.format(frame_name)}')]
        else:
            gt_paths = [osp.join(gt_path_root, clip_name, f'{self.filename_tmpl.format(frame_name)}')]
...
@PIPELINES.register_module()
class GenerateFrameIndicesEDVR_mix:
    ...
    def __call__(self, results):
        ...
        split_list=results['key'].split(os.sep)[::-1]
        frame_name,clip_name = int(split_list[0].split('.png')[0]),split_list[1]  # key example: 001/f001
        center_frame_idx = frame_name - self.idx_start_from
        last_idx = results['sequence_length'] - 1
        ...
        while (start_frame_idx < 0) or (end_frame_idx > last_idx):
        ...
        neighbor_list = list(range(center_frame_idx - num_half_frames * interval + self.idx_start_from, center_frame_idx + num_half_frames * interval + 1 + self.idx_start_from, interval))
        ...
        lq_path_root = results['lq_path']
        gt_path_root = results['gt_path']
        # print('------------',cprs15_path_root,clip_name, neighbor_list)

        cprs15_path = [
            osp.join(cprs15_path_root, clip_name, self.filename_tmpl.format(v))
            for v in neighbor_list
        ]
        cprs25_path = [
            osp.join(cprs25_path_root, clip_name, self.filename_tmpl.format(v))
            for v in neighbor_list
        ]
        cprs35_path = [
            osp.join(cprs35_path_root, clip_name, self.filename_tmpl.format(v))
            for v in neighbor_list
        ]
        ...
        gt_path = [osp.join(gt_path_root, clip_name, f'{frame_name}')]
        ...
        # print('$$$$$$$$$$$$',cprs15_path[5],neighbor_list,gt_path)
        # breakpoint()
...
@PIPELINES.register_module()
class GenerateFrameIndicesEDVR_mix_ldp(GenerateFrameIndicesEDVR_mix):
    ...
    def __call__(self, results):
        ...
        # add the corresponding file paths
        cprs15_path_root,left_pqf_idx15, right_pqf_idx15 = results['cprs15_path']
        cprs25_path_root,left_pqf_idx25, right_pqf_idx25 = results['cprs25_path']
        cprs35_path_root,left_pqf_idx35, right_pqf_idx35 = results['cprs35_path']
        lq_path_root,left_pqf_idxx4, right_pqf_idxx4 = results['lq_path']
        gt_path_root = results['gt_path']
        # print('##############',cprs15_path_root,clip_name,left_pqf_idx15,frame_name)
        # breakpoint()
        cprs15_path = [osp.join(cprs15_path_root, clip_name, self.filename_tmpl.format(left_pqf_idx15)),
        osp.join(cprs15_path_root, clip_name, self.filename_tmpl.format(frame_name)),
        osp.join(cprs15_path_root, clip_name, self.filename_tmpl.format(right_pqf_idx15))]
 
        cprs25_path = [osp.join(cprs25_path_root, clip_name, self.filename_tmpl.format(left_pqf_idx25)),
        osp.join(cprs25_path_root, clip_name, self.filename_tmpl.format(frame_name)),
        osp.join(cprs25_path_root, clip_name, self.filename_tmpl.format(right_pqf_idx25))]

        cprs35_path = [osp.join(cprs35_path_root, clip_name, self.filename_tmpl.format(left_pqf_idx35)),
        osp.join(cprs35_path_root, clip_name, self.filename_tmpl.format(frame_name)),
        osp.join(cprs35_path_root, clip_name, self.filename_tmpl.format(right_pqf_idx35))]

        lq_path = [osp.join(lq_path_root, clip_name, self.filename_tmpl.format(left_pqf_idxx4)),
        osp.join(lq_path_root, clip_name, self.filename_tmpl.format(frame_name)),
        osp.join(lq_path_root, clip_name, self.filename_tmpl.format(right_pqf_idxx4))]

        gt_path = [osp.join(gt_path_root, clip_name, self.filename_tmpl.format(frame_name))]
...
@PIPELINES.register_module()
class GenerateFrameIndicesEDVR_mix_ldp_test(GenerateFrameIndicesEDVR_mix):
    ...
    def __call__(self, results):
        ...
        if 'davis' in gt_path_root:
            filename_tmpl_gt='{:05d}.jpg'
            gt_path = [osp.join(gt_path_root, clip_name, filename_tmpl_gt.format(frame_name))]
        else:
            gt_path = [osp.join(gt_path_root, clip_name, self.filename_tmpl.format(frame_name))]
...
@PIPELINES.register_module()
class GenerateFrameIndicesEDVR_mix_ldp_test_kitti(GenerateFrameIndicesEDVR_mix):
 
 
    def __call__(self, results):
        # breakpoint()
        # print('------------',results['key'])
        clip_name,frame_name = results['key'].split('_')  # key example: 001/f001
        frame_name=int(frame_name.split('.png')[0])
        ...
        lq_path = [osp.join(lq_path_root, f'{clip_name}_{self.filename_tmpl.format(left_pqf_idxx4)}'),
        osp.join(lq_path_root, f'{clip_name}_{self.filename_tmpl.format(frame_name)}'),
        osp.join(lq_path_root, f'{clip_name}_{self.filename_tmpl.format(right_pqf_idxx4)}')]
        filename_tmpl_gt='{:02d}.png'
        gt_path = [osp.join(gt_path_root, f'{clip_name}_{filename_tmpl_gt.format(frame_name)}')]
...
@PIPELINES.register_module()
class GenerateFrameIndicesEDVR_mix_mwgan_test_kitti(GenerateFrameIndicesEDVR_mix):
    ...
    def __call__(self, results):
        ...
        # print('------------',results['lq_path'], frame_name)
        if frame_name==10:

            lq_path = [osp.join(lq_path_root,  f'{clip_name}_10.png'), osp.join(lq_path_root,  f'{clip_name}_11.png'), 
            osp.join(lq_path_root,  f'{clip_name}_10.png'), 
            osp.join(lq_path_root,  f'{clip_name}_11.png'), osp.join(lq_path_root,  f'{clip_name}_10.png')]
        elif frame_name==11:

            lq_path = [osp.join(lq_path_root,  f'{clip_name}_11.png'), osp.join(lq_path_root,  f'{clip_name}_10.png'), 
            osp.join(lq_path_root,  f'{clip_name}_11.png'), 
            osp.join(lq_path_root,  f'{clip_name}_10.png'), osp.join(lq_path_root,  f'{clip_name}_11.png')]
...
@PIPELINES.register_module()
class GenerateSegmentIndices_Mix_Compress:
...
@PIPELINES.register_module()
class GenerateSegmentIndices_Mix_Compress_KITT(GenerateSegmentIndices_Mix_Compress):
    ...
    def __call__(self, results):
        ...
        cprs15_path = [ f'{cprs15_path_root}/{clip_name}_{v}'
            for v in neighbor_list
        ]
        cprs25_path = [ f'{cprs25_path_root}/{clip_name}_{v}'
            for v in neighbor_list
        ]
        cprs35_path = [ f'{cprs35_path_root}/{clip_name}_{v}'
            for v in neighbor_list
        ]
        lq_path = [ f'{lq_path_root}/{clip_name}_{v}'
            for v in neighbor_list
        ]
        
        gt_path = [ f'{gt_path_root}/{clip_name}_{v}'
            for v in neighbor_list
        ]
...
class GenerateSegmentIndices:
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: A dict containing the processed data and information.
        """
        # key example: '000', 'calendar' (sequence name)    walk/000
        clip_name = results['key']
        interval = np.random.choice(self.interval_list)
        ...
        # add the corresponding file paths
        cprs_path_root = results['cprs_path']
        lq_path_root = results['lq_path']
        gt_path_root = results['gt_path']
        
        cprs_path = [
            osp.join(cprs_path_root, clip_name, self.filename_tmpl.format(v))
            for v in neighbor_list
        ]
        # for v in neighbor_list:
        #     print('!!!!!!!!!!!!!!', cprs_path_root, clip_name, self.filename_tmpl.format(v))  
...
@PIPELINES.register_module()
class GenerateSegmentIndices_LR(GenerateSegmentIndices):
...
@PIPELINES.register_module()
class GenerateSegmentIndicesVid4(GenerateSegmentIndices):
    """Generate frame indices for a segment. It also performs temporal
    augmention with random interval.
    ...
    def __call__(self, results):
        ...
        for key in self.keys:
            if key=='mvs':
                if isinstance(results[key], list): 
                    for v in range(len(results[key])):
                        for v_key in results[key][v].keys():
                            results[key][v][v_key]=results[key][v][v_key] + results[key][v][v_key][::-1]# remember to revser the flow
                else:
