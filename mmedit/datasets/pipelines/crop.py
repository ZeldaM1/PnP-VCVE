...
class PairedRandomCrop:
    ...
    def __call__(self, results):
        ...
        # cprs_is_list = isinstance(results['cprs'], list)
        # if not cprs_is_list:
        #     results['cprs'] = [results['cprs']]
        ...
        # h_cprs, w_cprs, _ = results['cprs'][0].shape
...
@PIPELINES.register_module()
class PairedRandomCrop_mv(PairedRandomCrop):
    def __call__(self, results):
        scale = results['scale']
        lq_patch_size = self.gt_patch_size // scale
        ...
        mv_is_list = isinstance(results['mvs'], list)
        if not mv_is_list:
            results['mvs'] = [results['mvs']]

        if 'partitions' in results.keys():
            par_is_list = isinstance(results['partitions'], list)
            if not par_is_list:
                results['partitions'] = [results['partitions']]
        ...
        results['mvs'] = [
            v[top:top + lq_patch_size, left:left + lq_patch_size, ...]
            for v in results['mvs']
        ]
        if 'partitions' in results.keys():
            results['partitions'] = [
                v[top:top + lq_patch_size, left:left + lq_patch_size, ...]
                for v in results['partitions']
            ]
        ...
        if 'partitions' in results.keys():
            if not par_is_list:
                results['partitions'] = results['partitions'][0]
