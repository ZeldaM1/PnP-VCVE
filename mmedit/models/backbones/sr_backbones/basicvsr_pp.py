...
class BasicVSRPlusPlus(nn.Module):
    ...
    def __init__(self,
        ...
        # import pdb
        # pdb.set_trace()
        # macs, params = get_model_complexity_info(self.deform_align['backward_1'], (196, 64,112), as_strings=True, print_per_layer_stat=True, verbose=True)
        # print('{:<30}  {:<8}/{:<8}'.format('parameters/complexity: ',params,macs))
    ...
    def propagate(self, feats, flows, module_name, slice_type):
    ...
    def forward(self, lqs,QPs=None, slices=None, mvs=None, base_QPs=None,par_map=None):
        # hqs=[]
        # for i in range(lqs.size()[1]):
        #     hqs.append(F.interpolate(lqs[:,i,...], scale_factor=4, mode='bicubic', align_corners=True))        
        # return torch.cat(hqs,dim=0).unsqueeze(0)
        ...
        for iter_ in [1, 2]:
            for direction in ['backward', 'forward']:
                ...
                feats = self.propagate(feats, flows, module, slice_type=None)
                if self.cpu_cache:
                    del flows
                    torch.cuda.empty_cache()
