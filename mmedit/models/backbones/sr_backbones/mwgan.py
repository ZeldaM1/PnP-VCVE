...
from mmedit.models.common import flow_warp,make_layer
...
class DenseMWNet_Mini_PSNR(nn.Module):
    ...
    def forward(self, x, QPs=None, slices=None, mvs=None, base_QPs=None, partitions=None):
        B, N, C, H, W = x.size()  # N video frames
        # temporal zero padding, same as 3DCNN
        # padding_fea = x.new_zeros(B, pad_len, C, H, W)
        # x = torch.cat([padding_fea, x, padding_fea], dim=1)
        ...
        if not self.training:
            pic_height, pic_width = x.shape[3], x.shape[4]
            # pad if necessary
            padding_l, padding_r, padding_t, padding_b = get_padding_size(pic_height, pic_width, 16)
            cur_input_list=[]
            for idx in range(N):
                cur_input_list.append(F.pad(x[:,idx,...],(padding_l, padding_r, padding_t, padding_b),mode="replicate",).unsqueeze(1))
            x=torch.cat(cur_input_list, dim=1)
        ...
        if not self.training:
            out = F.pad(out, (-padding_l, -padding_r, -padding_t, -padding_b))#yuv444
        return out
